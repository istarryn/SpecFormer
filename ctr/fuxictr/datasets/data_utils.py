...
def data_generator(feature_encoder, stage="both", train_data=None, valid_data=None, test_data=None, use_llm_emb=False, nlp_fields=0,
                   train_index_seq=None, valid_index_seq=None, test_index_seq=None,
                   train_dot_sum=None, valid_dot_sum=None, test_dot_sum=None,
    ...
    if feature_encoder.version == 'tensorflow':
        ...
        from ..pytorch.data_generator import DataGenerator, DataGenerator4, DataGenerator_idx, DataGenerator_fm
    train_gen = None
    valid_gen = None
    test_gen = None
    ...
    if stage in ["both", "train"]:
        ...
        if use_llm_emb is True:
            ...
            for batch_data in train_gen:
                # batch_data: list[X,y], len=2,  
                print(f"batch_data[0].shape={batch_data[0].shape}") 
                print(f"batch_data[0][0]={batch_data[0][0]}") 
                # batch_data[1].shape=X=tensor[B,fields]
                # batch_data[0][0]=tensor([1994.,    2.,    5.,  633.,  633.], dtype=torch.float64)
                print(f"batch_data[1].shape={batch_data[1].shape}")
                print(f"batch_data[0][0]={batch_data[1][0]}")
                # batch_data[1].shape=y=tensor[B,1]
                # batch_data[1][0]=1.0
                exit()
            '''

            ## index: 先修改array，然后在generator中分成batch，最后自动返回tensor          
            train_gen = DataGenerator_idx(train_array, train_index_seq, batch_size=batch_size, shuffle=shuffle, neg_samples=neg_samples, nlp_fields=nlp_fields, **kwargs) 
            valid_gen = DataGenerator_idx(valid_array, valid_index_seq, batch_size=batch_size, shuffle=False, neg_samples=-1, **kwargs)
            '''
            for batch_data in train_gen:
                print("len(batch_data)=", len(batch_data))
                # batch_data: list[X,y,idx], len=3,  
                print(f"batch_data[0].shape={batch_data[0].shape}") 
                print(f"batch_data[0][0]={batch_data[0][0]}") 
                # batch_data[1].shape=X=tensor[B,fields]
                # batch_data[0][0]=tensor([1994.,    2.,    5.,  633.,  633.], dtype=torch.float64)
                print(f"batch_data[1].shape={batch_data[1].shape}")
                print(f"batch_data[1][0]={batch_data[1][0]}")
                # batch_data[1].shape=y=tensor[B,1]
                # batch_data[1][0]=1.0
                print(f"batch_data[2].shape={batch_data[2].shape}")
                print(f"batch_data[2][0]={batch_data[2][0]}")
                # batch_data[2].shape=idx=tensor[B,K]
                # batch_data[2][0]=tensor([726, 478, 883, 520])
                exit()
            '''
        else:
            train_gen = DataGenerator(train_array, batch_size=batch_size, shuffle=shuffle, neg_samples=neg_samples, **kwargs)
    ...
    if stage in ["both", "test"]:
        ...
        if use_llm_emb is True:
            ...
            ## index
            test_gen = DataGenerator_idx(test_array, test_index_seq, batch_size=batch_size, shuffle=False, neg_samples=-1, **kwargs)
