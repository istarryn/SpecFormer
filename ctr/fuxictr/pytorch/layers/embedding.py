...
class EmbeddingLayer_v3(nn.Module):
    ...
    def forward(self, X):
        feature_emb_dict = self.embedding_layer(X) # [B, fields] (float, not embedded) --> dict {fields: [B, dim]}
        feature_emb = torch.stack(self.embedding_layer.dict2list(feature_emb_dict), dim=1) # dict {fields: [B, dim]} --> [B, fields, dim]
        if self.dropout is not None:
            feature_emb = self.dropout(feature_emb)
        return feature_emb
...
class EmbeddingDictLayer(nn.Module):
    def __init__(self, 
        ...
        for feature, feature_spec in self._feature_map.feature_specs.items():
            # feature= UserID
            # feature_spec= {'source': '', 'type': 'categorical', 'min_categr_count': 1, 'vocab_size': 5401, 'index': 0}
            ...
            if self.is_required(feature):
                ...
                if feature_spec["type"] == "numeric":
                    ...
                    if feature not in self.embedding_layer: 
                        ...
                        # print("padding_idx = ", padding_idx) # None
                        ...
                        # print("pretrained_emb", "pretrained_emb" in feature_spec) # False
        ...
        # (UserID): Embedding(5401, 32)
        # (Gender): Embedding(3, 32)
        # (Age): Embedding(8, 32)
        # (Occupation): Embedding(22, 32)
        # (MovieID): Embedding(3663, 32)
        # (Title): Embedding(3663, 32)
        # (Genres): Embedding(302, 32)
        # exit()
    ...
    def is_required(self, feature):
        ...
        # print("self.required_feature_columns = ", self.required_feature_columns)
        ...
        for feature, feature_spec in self._feature_map.feature_specs.items(): 
            ...
            if feature in self.embedding_layer:
                if feature_spec["type"] == "numeric":
                    ...
                    inp = X[:, feature_spec["index"]].long() # [B]
                    embedding_vec = self.embedding_layer[feature](inp) # [B, dim]
                    # print("inp=",inp[0])
                    # print("embedding_vec=",embedding_vec[0])                    
