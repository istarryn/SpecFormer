...
class BaseModel(nn.Module):
    def __init__(self, 
        ...
        self.llm_emb_path2 = llm_emb_path2
        ...
        self.llm_emb2 = llm_emb2

        self.checkpoint = os.path.abspath(os.path.join(self.model_dir, self.model_id + "_wd" + str(self.weight_decay) + \
            "_ls" + str(self.lambda_loss) + "_fm" + str(self.lambda_llm) + \
            "_" + str(self.llm_emb_path2).replace('/', '_') + "_model.ckpt"))
        # from datetime import datetime
        # current_time = datetime.now().strftime("%Y%m%d%H%M%S%f")
        # self.checkpoint = os.path.abspath(os.path.join(self.model_dir, self.model_id + "_" + current_time + "_model.ckpt"))

    def compile(self, optimizer, loss, lr=1e-3):
        try:
            self.optimizer = set_optimizer(optimizer)(self.parameters(), lr=lr, weight_decay=self.weight_decay)
        except:
    ...
    def inputs_to_device_llmcf(self, inputs):
        X, y, index_tensor = inputs
        X = X.to(self.device)
        y = y.float().view(-1, 1).to(self.device)
        self.batch_size = y.size(0)
        # index
        index_tensor = index_tensor.to(self.device)
        return X, y, index_tensor

    def inputs_to_device2(self, inputs):
        X, y, llm_emb = inputs
        X = X.to(self.device)
        y = y.float().view(-1, 1).to(self.device)
        self.batch_size = y.size(0)
    ...
    def inputs_to_device_fm(self, inputs):
        X, y, llm_dot_sum = inputs
        X = X.to(self.device)
        y = y.float().view(-1, 1).to(self.device)
        self.batch_size = y.size(0)
        
        llm_dot_sum = llm_dot_sum.to(self.device)
        return X, y, llm_dot_sum

    def inputs_to_device3(self, inputs):
        X, y, index = inputs
        X = X.to(self.device)
        y = y.float().view(-1, 1).to(self.device)
    ...
    def train_on_epoch(self, data_generator, epoch):
        ...
        for batch_index, batch_data in enumerate(batch_iterator):
            ...
            if self.adding_mode == 0:
                return_dict = model.forward(batch_data)
            else:
                return_dict = model.forward_llm(batch_data, self.lambda_loss, self.llm_emb2, self.lambda_llm, self.temperature, self.nlp_fields)
                # return_dict = model.forward_ctrl(batch_data, self.lambda_loss, self.llm_emb2, self.lambda_llm, self.temperature, self.nlp_fields)
            # import pdb
            # pdb.set_trace()
        ...
        e1 = time.time()
        print(f"training one epoch cost {e1 - s1} s")  
        ...
        with torch.no_grad():
            ...
            for batch_data in data_generator:
                if self.adding_mode == 0:
                    ...
                    return_dict = self.forward_llm(batch_data, self.lambda_loss, self.llm_emb2, self.lambda_llm, self.temperature, self.nlp_fields)
                    # return_dict = self.forward_ctrl(batch_data, -1, self.llm_emb2, self.lambda_llm, self.temperature, self.nlp_fields)

                    # if ip_mean is None:
                    #     ip_mean = ip_mean_batch
                    # else:
                    #     ip_mean = (ip_mean + ip_mean_batch) / 2

                y_pred.extend(return_dict["y_pred"].data.cpu().numpy().reshape(-1))
                y_true.extend(batch_data[1].data.cpu().numpy().reshape(-1))

            # torch.save(ip_mean, "./ana/auc/ip_mean_rank_movie.pt")

            y_pred = np.array(y_pred, np.float64)
            y_true = np.array(y_true, np.float64)
            val_logs = evaluate_metrics(y_true, y_pred, self._validation_metrics)

            # val_logs: {'logloss': 0.6004048833354569, 'AUC': 0.6296750837514499}
            import nni
            accuracy = {                   
                'default': val_logs['AUC'],
                'logloss': val_logs['logloss'],
            }
